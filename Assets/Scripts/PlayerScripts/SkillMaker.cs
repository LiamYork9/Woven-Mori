...
public class SkillMaker : MonoBehaviour
{
    ...
    public void SkillCreation()
    {
        ...
        Skill StaticShock = new Skill(SkillId.StaticShock, "Static Shock", Element.Air, Target.mutipleEnemy, Category.Attack, 70, 2, "Hit mutiple guys with lighting", 0, 1).Attr(new DamageAttr(20,DamageType.Magic,Element.Air));
        ...
        Skill BurnAway = new Skill(SkillId.BurnAway, "Burn Away",Element.Fire, Target.single,Category.Attack,0,2,"Lower targets Defense",0,1).Attr(new StatBoostConAttr(Stats.Defence, -5)).Attr(new DamageAttr(15,DamageType.Physical,Element.Fire));
        madeSkills.Add(BurnAway);
        Skill TriSlash = new Skill(SkillId.TriSlash, "Tri-Slash",Element.Fate,Target.single,Category.Attack,0,3,"This Attack hits 3 times each with a uniqe element",0,3).Attr(new DamageAttr(10,DamageType.Physical,Element.Fire)).Attr(new DamageAttr(10,DamageType.Physical,Element.Water)).Attr(new DamageAttr(10,DamageType.Physical,Element.Earth));
        madeSkills.Add(TriSlash);
        Skill FiredUp = new Skill(SkillId.FiredUp, "Fired Up", Element.Fire, Target.self, Category.Buff,0,3,"Greatly Boost Attack",0,0).Attr(new StatBoostConAttr(Stats.Attack, 20,2));
